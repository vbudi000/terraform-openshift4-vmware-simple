...
resource "vsphere_virtual_machine" "control_plane" {
  ...
  # VM Specifications
  ####
  count            = "${var.datastore != "" ? var.control_plane["count"] : 0 }"
  ...
  disk {
      label            = "${format("${lower(var.name)}-master%02d-boot.vmdk", count.index + 1) }"
  ...
  disk {
      label            = "${format("${lower(var.name)}-master%02d_disk1.vmdk", count.index + 1) }"
...
resource "vsphere_virtual_machine" "control_plane_ds_cluster" {
  depends_on = [
    "null_resource.haproxy_cfg",
    "null_resource.generate_ignition_config"
  ]
  ...
  # VM Specifications
  ####
  count            = "${var.datastore_cluster != "" ? var.control_plane["count"] : 0 }"
  resource_pool_id = "${data.vsphere_resource_pool.pool.id}"
